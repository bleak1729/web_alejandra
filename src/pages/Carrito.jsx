...
export default function Carrito() {
  ...
  if (!user) return (
    <div className="container-app py-20 text-center">
      <p className="text-tinta/70 mb-4">Ingresa para ver tu carrito.</p>
  ...
  return (
    <div className="container-app py-12 grid md:grid-cols-3 gap-10">
      <div className="md:col-span-2 space-y-4">
        ...
        {items.map((it) => (
          <div key={it.productoId} className="card flex gap-4 p-4 items-center">
            <div className="w-20 h-20 rounded-lg bg-crema-100 overflow-hidden flex-shrink-0">
            ...
            <button onClick={() => removeFromCart(it.productoId)} className="text-tinta/40 hover:text-rosa-500 text-xl">×</button>
          </div>
        ))}
      </div>
      <aside className="card p-6 h-fit sticky top-24">
        ...
        <button onClick={() => nav('/checkout')} className="btn-primary w-full mt-6">
          Proceder al pago
        </button>
      </aside>
    </div>
